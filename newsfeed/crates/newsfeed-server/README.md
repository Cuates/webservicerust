...
- **Security Middleware**:
  ...
  - `ip_extractor::secure_ip_extractor`: Acts as a secure proxy fallback middleware for rate-limiting behind load balancers.
...
- **Graceful Shutdown**: Listens for SIGINT (Ctrl+C) and OS signals cross-platform (including Windows) and terminates active connections gracefully.
