...
- **`ExtractParams` / `CudParams`**: Strongly typed structs mapped from incoming JSON payloads or URL Query parameters. Enforces `#[serde(deny_unknown_fields)]` to reject malformed or inflated requests with structured validation errors.
