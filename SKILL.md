...
<!-- markdownlint-disable MD013 MD022 MD003 MD041 -->
...
3. **Strict Crate Boundaries**:
   - `newsfeed-server` handles ONLY HTTP transport (Extractors, Responses, Middleware) and HTTP payload validation.
   - `newsfeed-service` handles ONLY business logic orchestration.
...
6. **Zero-Trust Validation**: Never trust incoming HTTP payloads. All data must pass through `ExtractParams` or `CudParams` validation natively at the `newsfeed-server` boundary before reaching `newsfeed-service`. Payloads must not exceed the strict 1000-item batch limit, and all dates must strictly conform to `RFC3339`.
7. **Tooling**: Always use `cargo make` (defined in `Makefile.toml`) for running checks, tests (`cargo make test-coverage`), and builds, rather than raw `cargo` commands. Ensure the strict pre-commit hygiene pipeline (`check`, `machete`, `fix`, `audit`, `test-coverage`, `lint-docs`) passes before committing.
8. **Test Coverage**: Ensure any new logic is covered by unit or integration tests. The CI pipeline will reject commits if the strict coverage thresholds (>99% line and function coverage) drop below requirements.
